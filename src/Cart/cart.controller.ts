...
export class cartController {
  ...
  public CreateNewCart = async (req: any | Request, res: Response) => {
    try {
      ...
      const newProduct = await productService.getSingleProduct(
        req.body.productId
      );
      ...
      if (!newCart) {
        ...
        return res
          .status(200)
          .send({ message: "product added succesfully", newProductCart });
      } else {
        ...
        return res
          .status(200)
          .send({ message: "cart created succesfully", newCart });
      }
    } catch (err: any) {
      console.error(err.message);
  ...
  public removingItemsFromCart = async (req: any | Request, res: Response) => {
    const updatedCart = await service.findCartById(req.params.id);
    if (!updatedCart) {
    ...
    const unitPrice = Number(
      updatedCart.totalPrice / updatedCart.quantityInStock
    );
