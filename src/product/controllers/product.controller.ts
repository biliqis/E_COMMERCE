...
class ProductController {
  ...
  public createProduct = async (
    ...
    try {
      ...
      createProductDetails.userId = (req as any).user._id;
  ...
  public getAllProduct = async (req: Request, res: Response) => {
	...
	return res.status(200).json({message:'this are the list of products in the database', getProduct})
  };
  ...
  public search = async (req: Request, res: Response) => {
    ...
    if (product.length === 0) {
      return res
        .status(400)
        .json({ message: "no result found for this product" });
    }
